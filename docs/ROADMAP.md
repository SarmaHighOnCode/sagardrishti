...
- [ ] WSL2 + Docker + micromamba working on **both** machines, GPU verified *(everyone)* — **partial.** Docker is up and verified working on one machine as of 8 September 2026 (`db` container healthy, migrations applied and tested live). WSL2 is still broken on that same machine (`REGDB_E_CLASSNOTREG`); micromamba/GPU not yet checked; neither confirmed on the second machine
...
- [x] **`aisgen` producing valid AIS**, written through the shared `packages/go/store` path *(AIS lead)* — the `internal/` visibility fix landed (store/aoi/quality promoted per [ADR 0005](adr/0005-go-for-the-ais-data-plane.md)); `aisgen` generates lane-following traffic with labelled discharge ground truth and confounders. **Live-verified against a real Postgres, 8 September 2026** (`docker compose --profile aisgen run --rm aisgen` — 33k+ rows written, table truncated after) — this run caught and fixed a real bug in `packages/go/store` that would have silently broken `aisd`'s first live write too. See `docs/HANDOVER.md` task G
...
- [x] PostGIS schema stable and migrated *(Integration)* — migrations `001` and `002` both live-verified against a real Postgres (8 September 2026): all 12 tables, indexes, FKs and PostGIS geometry columns confirmed present, and the `evidence_dossiers` partial-unique-index behavior specifically tested (rejects a second `is_current=true` row for one detection, allows a second `is_current=false` one). See `db/README.md`. **"Stable" still only means the schema is correct** — the tables themselves are empty; nothing has recorded real AIS or ingested a real scene yet
