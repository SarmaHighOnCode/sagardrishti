...
- [x] **`aisgen` producing valid AIS**, written through the shared `packages/go/store` path *(AIS lead)* — the `internal/` visibility fix landed (store/aoi/quality promoted per [ADR 0005](adr/0005-go-for-the-ais-data-plane.md)); `aisgen` generates lane-following traffic with labelled discharge ground truth and confounders, 32 tests, dry-run verified end to end. **Not yet run against a live database** — Docker still down at time of writing
