...
- ~~Never run against a live database~~ — **done, 8 September 2026.** `docker compose --profile aisgen run --rm aisgen` writes real rows into `ais_positions`/`ais_static` (33,094 positions / 1,037 statics across 46 vessels in one default run, `source='aisgen'`). Table truncated afterward — same "verified, not left populated" convention as Task A.

  **This live run found a real bug, in `packages/go/store`, not in `aisgen` itself:** `copyUpsert`'s `CREATE TEMP TABLE (LIKE target)` copies column types and `NOT NULL` constraints but **not** `DEFAULT` expressions. `ais_positions`/`ais_static` both have `recorded_at TIMESTAMPTZ NOT NULL DEFAULT now()`, and the writer deliberately never supplies that column — so the staging table inherited the `NOT NULL` without the default that satisfies it, and every single COPY failed. This is the exact same code path `aisd` will hit the first time it ever writes anything for real; it was never caught because `store_test.go` could only exercise the nil-pool no-op case before a live database existed at all. Fixed with `LIKE target INCLUDING DEFAULTS`. `packages/go/store/store_integration_test.go` is a new, real regression test for this — skipped unless `SAGAR_TEST_DATABASE_URL` is set (no Postgres in CI), but genuinely passes against the compose `db` service. Run it: `SAGAR_TEST_DATABASE_URL=postgresql://sagar:change_me_locally@localhost:5432/sagardrishti go test ./store/... -run LiveDatabase -v` from `packages/go` — mind the port-5432 gotcha in `db/README.md` if that fails with an auth error despite the container being healthy.

  Also fixed along the way: `aisd.Dockerfile` didn't build at all — it only copied `services/aisd/`, but `aisd`'s `go.mod` now depends on `packages/go` via a relative `replace` directive that needs `packages/go/` present in the build context too. Fixed, and a new `infra/docker/aisgen.Dockerfile` + a profiled `aisgen` service in `docker-compose.yml` follow the same corrected pattern — `docker compose --profile aisgen run --rm aisgen` is now the one-line way to do everything above.
